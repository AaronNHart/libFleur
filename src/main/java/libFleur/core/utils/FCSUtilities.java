...
import libFleur.core.data.Subset;
import libFleur.core.fcs.FCSFileReader;
import libFleur.core.fcs.DimensionTypes;
import libFleur.core.singlets.PuleProperties;
...
public class FCSUtilities {
  ...
  public static final String PROP_KEY_PREVIEW_FRAME = "Fleur Preview Frame";
  ...
  public static FCSFrame createSummaryFrame(List<FCSFrame> fcsList, Integer maxEventsPerFrame) {
    ...
    if (optReturn.isPresent()) {
      ...
      df.setDisplayName(PROP_KEY_PREVIEW_FRAME);
    } else {
      return null;
    }
  ...
  /**
   * 
   * @param dimensions list of FCSDimensions
   ...
   */
  private static FCSDimension findDimensionType(ArrayList<FCSDimension> dimensions,
      PuleProperties pulseTypes) {
    FCSDimension foundDimension = null;
    if (!dimensions.isEmpty()) {
      Optional<FCSDimension> optionalDimension = dimensions.stream()
